...
var ErrorTaskPoolInvalidConfiguration = fmt.Errorf("TaskPool: Invalid configuration")
...
type Option struct {
	// 创建池对象时，预先开启的worker(协程)数量，如果为0，则不预先开启。
	InitWorkerNum int
...
func validate(option Option) error {
	if option.InitWorkerNum < 0 {
		return ErrorTaskPoolInvalidConfiguration
	}
